Shader "BeresnevGames/CosmoCompostGroundLit"
{
    Properties
    {
        ...
        _AdditionalMap ("Additional Map", 2D) = "white"{} 
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            TEXTURE2D(_GlobalLevelDataTexture);
            SAMPLER(sampler_GlobalLevelDataTexture);
            ...
            {
                half4 result;
                half3 halfSize = (_LocationMax - _LocationMin) * 0.5;
                half u = (IN.positionWS.x - _LocationMin.x + halfSize.x) / (_LocationMax.x - _LocationMin.x);
                half v = (IN.positionWS.z - _LocationMin.z + halfSize.z) / (_LocationMax.z - _LocationMin.z);
                half2 globalUV = half2(u, v);
                
                
                half4 albedo = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, IN.uv);
                half4 globalTextureData = SAMPLE_TEXTURE2D(_GlobalLevelDataTexture, sampler_GlobalLevelDataTexture, globalUV);
                half d = max(0.0,dot(IN.normalWS, half3(0.0,1.0,0.0)));
                d = step(0.5, d);
                half3 soil = lerp(1.0, half3(0.2, 0.05, 0.1), step(0.3h,globalTextureData.b) * d);
                albedo.xyz *= lerp(soil, albedo.xyz * half3(0.3, 0.4, 0.3), step(0.1h,globalTextureData.r) * d);
                ...
                surface.smoothness = _Smoothness * smoothnessMask * soil;

                #if defined (_NORMALMAP)
               
                //normalTS.xy = additionalMaps.rg * 2.0 - 1.0;
                //normalTS.z = sqrt(max(0, 1 - (normalTS.x * normalTS.x) - (normalTS.y * normalTS.y)));// max(1.0e-16, sqrt(1.0 - saturate(dot(normalTS.xy, normalTS.xy))));
